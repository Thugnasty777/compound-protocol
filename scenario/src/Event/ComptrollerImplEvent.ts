...
async function becomeG3(
  ...
  let invokation = await invoke(
    world,
    comptrollerImpl.methods._become(unitroller._address, vtxRate, compMarkets, otherMarkets),
    from,
    ComptrollerErrorReporter
  );
...
export function comptrollerImplCommands() {
  return [
    ...
    new Command<{
      ...
      (world, from, { unitroller, comptrollerImpl, vtxRate, compMarkets, otherMarkets }) => {
        return becomeG3(world, from, comptrollerImpl, unitroller, vtxRate.encode(), compMarkets.val.map(a => a.val), otherMarkets.val.map(a => a.val))
      },
      { namePos: 1 }
    ),
