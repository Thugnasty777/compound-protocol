...
describe('Comp', () => {
  ...
  describe('delegateBySig', () => {
    ...
    it('reverts if the signatory is invalid', async () => {
      ...
      await expect(send(comp, 'delegateBySig', [delegatee, nonce, expiry, 0, '0xbad', '0xbad'])).rejects.toRevert("revert Vtx::delegateBySig: invalid signature");
    });
    ...
    it('reverts if the nonce is bad ', async () => {
      ...
      await expect(send(comp, 'delegateBySig', [delegatee, nonce, expiry, v, r, s])).rejects.toRevert("revert Vtx::delegateBySig: invalid nonce");
    });

    it('reverts if the signature has expired', async () => {
      const delegatee = root, nonce = 0, expiry = 0;
      const { v, r, s } = EIP712.sign(Domain(comp), 'Delegation', { delegatee, nonce, expiry }, Types, unlockedAccount(a1).secretKey);
      await expect(send(comp, 'delegateBySig', [delegatee, nonce, expiry, v, r, s])).rejects.toRevert("revert Vtx::delegateBySig: signature expired");
    });
  ...
  describe('getPriorVotes', () => {
    it('reverts if block number >= current block', async () => {
      await expect(call(comp, 'getPriorVotes', [a1, 5e10])).rejects.toRevert("revert Vtx::getPriorVotes: not yet determined");
    });
