...
function printUsage() {
	console.log(`
	usage: npx saddle script chainlinkProxy:deploy {config}
...
function getConfig(configArgs) {
	...
	let res = {
		ethUsdChainlinkAggregatorAddress: getString(config, 'ethUsdChainlinkAggregatorAddress', true)
	}
...
(async function () {
	...
	console.log(`Deploying chainlinkProxy with ${JSON.stringify(conf)}`)

	let deployArgs = [conf.ethUsdChainlinkAggregatorAddress]
	let contract = await saddle.deploy('ChainlinkPriceOracleProxy', deployArgs)
    ...
    await saddle.verify(etherscanApiKey, contract._address, 'ChainlinkPriceOracleProxy', deployArgs, 0);
